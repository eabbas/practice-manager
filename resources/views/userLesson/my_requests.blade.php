...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
  <main class="w-full max-w-lg bg-white shadow-md rounded-2xl p-6 mr-150">
    ...
    <form id="enrollForm" class="space-y-4" action="#" method="POST" onsubmit="handleSubmit(event)">
      @csrf
      <div>
        <?php //dd($lesson1[0]); ?>
        <input type="hidden" name="lesson_id" value="{{$userLesson->lesson_id}}" class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm p-3">
      </div>
      <div>
      ...
      <div>
        ...
        <input type="text" name="title" value="{{$userLesson->title}}" class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm p-3">
        </div>
      </div>
      {{-- @endforeach --}}
      <div>
