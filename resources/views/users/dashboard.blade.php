            ...
            <div class="w-full fixed top-0 right-0 z-10">
                <div class="w-full float-end lg:w-[calc(100%-265px)] py-3 hidden lg:flex flex-row-reverse px-5 backdrop-blur-sm shadowHeader relative z-20">
                    <div class="w-full flex flex-row-reverse justify-between">
                        <div class="w-full flex justify-end items-center bg-white p-4 rounded-xl shadow mt-1">
                            <h1 class="text-lg font-semibold text-gray-700 my-2"> 👩‍🎓خوش آمدید، {{Auth::user()->name}} {{Auth::user()->family}} </h1>
                            <div class="flex items-center gap-4">
                                <img src="{{ asset('assets/img/user.png') }}"
                                    src="https://i.pravatar.cc/40?img=47"
                                    alt="avatar"
                                    class="w-10 h-10 rounded-full border-2 border-primary mr-3"/>
                                    ...
                                    <li class="color__a text-[#4B5675]">
                                    <a href="{{ route('user.profile',Auth::user()) }}" class="py-2"> پروفایل کاربری</a>
                                    </li>
                                    </ul>
                                 </div>
                                 </div>
